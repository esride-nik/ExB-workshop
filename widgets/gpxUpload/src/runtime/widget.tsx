...
export default function (props: AllWidgetProps<Config>) {
    ...
    }, [bufferPolygon, color, gpxLayer]);

    // use buffer to fill output data source
    useEffect(() => {
    ...
    const addTrackToMap = (esriFeatures: Graphic[]) => {
        const gpxLayer = new GraphicsLayer({
            listMode: 'hide',
            id: 'gpxLineString',
        });
        ...
        setGpxLayer(gpxLayer)

        // TODO: productive version: please verify array contents!
        createBuffer(
    ...
    const createBuffer = async (inputGeometries: Geometry[]) => {
        ...
        const bufferPolygon = buffers.length > 0 ? buffers[0] : null;
        if (bufferPolygon) {
            setBufferPolygon(bufferPolygon);
        }
    };
    ...
    if (!isConfigured()) {
        return (
            <h5>
                <FormattedMessage id="cfgDataSources" defaultMessage={defaultMessages.cfgDataSources} />
            </h5>
        );
    }
    const dropzoneStyle = {
    ...
    return (
        <div
            ...
            <h3>
                <FormattedMessage id="dropIt" defaultMessage={defaultMessages.dropIt} />
            </h3>
